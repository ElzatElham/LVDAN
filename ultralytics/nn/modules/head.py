...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        imgn = x[0].shape[0]
        x0_flat = x[0].view(imgn, -1)
        x1_flat = x[1].view(imgn, -1)
        x2_flat = x[2].view(imgn, -1)
        ...
        concatenated = torch.cat((x0_flat, x1_flat, x2_flat), dim=1)

        # 将连接后的样本展平为一维向量
        flattened = concatenated.view(imgn, -1)
