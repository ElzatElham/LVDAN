...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        if len(daerror) > 0:
            ...
            loss[3] = torch.sum(min_distances) / (tn - n_batch)

            loss[3] *= self.hyp.da

            l_dmt = dmt_loss(
                high_data_s=high_data_s,
                high_data_s_aug=high_data_s_aug,
                high_data_t=high_data_t,
                high_data_t_aug=high_data_t_aug,                
                low_data_s=low_data_s,
                low_data_s_aug=low_data_s_aug,
                low_data_t=low_data_t,
                low_data_t_aug=low_data_t_aug,                
                rho=0,
                sigma=1
                )
        ...
        if fg_mask.sum():
            ...
            for i in range(batch_size):
                if fg_mask[i].sum():
                    ...
                    if self.overlap:
                        gt_mask = torch.where(
                            masks[[i]] == (mask_idx + 1).view(-1, 1, 1), 1.0, 0.0
                        )
                    else:
