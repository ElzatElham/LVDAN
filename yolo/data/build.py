...
class CustomBatchSampler(Sampler):
    ...
    def __iter__(self):
        batch_size = next(self.batch_sizes, None)
        dataset_indices = torch.randperm(len(self.dataset)).tolist()  # 随机打乱索引
        while dataset_indices:
            current_batch_size = batch_size if batch_size else len(dataset_indices)
            yield dataset_indices[:current_batch_size]
            dataset_indices = dataset_indices[current_batch_size:]
