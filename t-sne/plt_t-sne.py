...
def parse_args():
    ...
    parser.add_argument('--Input_path', type=str, help='Path to the input image folder')
    parser.add_argument('--size', type=int, nargs='+', default=[2, 256], help='List of sizes for normalization')
...
def main(args):
    ...
    for size in sizes:
        ...
        tsne_2D = TSNE(n_components=2, init='pca',perplexity= 2,random_state=0) #调用TSNE
        ...
        for i in range(embedding.shape[0]):
            plt.scatter(embedding[i, 0], embedding[i, 1], c=colors[int(label[i])], s=80, marker='.')
